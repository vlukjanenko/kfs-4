...
int is_page_maped(uint32_t ptr) // вынести получение привязаной страницы в общую функию?
{
...
static void *__get_virt_page(void **start, void* end, uint32_t nbr)
{
	uint32_t i = 0;
	void *result = *start;
	while (*start < end && !is_page_maped((uint32_t)*start)) {
		if (i >= nbr) {
			return (result);
		}
		i++;
...
uint32_t *get_table_entry(void *page)
{
	uint32_t *pde = (uint32_t *) 0xfffff000;
	uint32_t pde_index = (uint32_t)page >> 22;
	uint32_t table_index = ((uint32_t)page & 0x3FF000) >> 12;
	uint32_t *table = (uint32_t *)((pde[pde_index] & 0xFFFFF000) + PAGE_OFFSET);

	printf("page %x table %x table[%u] %x\n", page, table, table_index, table[table_index]);
...
static void map_pages(void *pages, uint32_t *frames, uint32_t nbr)
{
	for (uint32_t i = 0; i < nbr; i++, pages += PAGE_SIZE) {
		*get_table_entry(pages) = frames[i] | 3;
	}
	refresh_map();
}
...
static void *get_page_in_highmem(uint32_t nbr) // делаем так чтоб она не брала странички в lowmem зоне
{
	...
	printf("Start = %#x\n", start);
	printf("End = %#x\n", end);
	printf("is maped start, %d\n", is_page_maped((uint32_t)start));

	while (start < end && pages == NULL) {
		if (!is_page_maped((uint32_t)start)) {
